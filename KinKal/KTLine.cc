/*
  KTLine inherits from TLine but we also want it be an instance of KKTrk. We
  dont need all the parameters and functions to be redefined. KTLine follows
  from TLine.
...
KTLine::KTLine(Vec3 const &pos0, Vec3 const &svel, double tmeas,
               TRange const &range, bool forcerange)
...
KTLine::KTLine(Vec4 const &pos40, Mom4 const &mom0, int charge,
               Vec3 const &bnom, TRange const &range)
    : KTLine(pos40.Vect(), (mom0.Vect() / mom0.E()) * CLHEP::c_light, pos40.T(),
             range) {
  bnom_ = bnom;
  pos40_ = pos40;
  mom_ = mom0;
...
KTLine::KTLine(PDATA::DVEC const &pvec, PDATA::DMAT const &pcov, double mass,
               int charge, Vec3 const &bnom, TRange const &trange)
    : KTLine(pvec, pcov) {
  bnom_ = bnom;
  mass_ = mass;
  charge_ = charge;
