...
TPoca<KTLine, TLine>::TPoca(KTLine const &ktline, TLine const &tline,
  ...
  double dptoca(std::numeric_limits<double>::max()),
      dstoca(std::numeric_limits<double>::max());
  ...
  if (status_ != TPoca::pocafailed) {
    ...
    // sign doca by angular momentum projected onto difference vector (same as
    // helix)
    double lsign = tline.dir()
                       .Cross(ktline.direction(partPoca_.T()))
                       .Dot(sensPoca_.Vect() - partPoca_.Vect());
    double dsign = copysign(1.0, lsign);
    doca_ = doca * dsign;

    // pre-compute some values needed for the derivative calculations
    Vec3 ddir;
    ddir = delta().Vect().Unit(); // direction vector along D(POCA) from traj 2
                                  // to 1 (line to ktline)
...
template <>
TPoca<PKTLine, TLine>::TPoca(PKTLine const &pktline, TLine const &tline,
                             TPocaHint const &hint, double precision)
  ...
  unsigned niter = 0;
  size_t oldindex =
      pktline.pieces().size(); // TODO --->do we want piecewise line fit?
  size_t index;
  if (hint.particleHint_) {
